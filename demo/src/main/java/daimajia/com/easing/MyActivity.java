...
public class MyActivity extends Activity {
    ...
    private DrawView mHistory,mSpeed;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSpeed = (DrawView)findViewById(R.id.speed);
        et_var = (EditText)findViewById(R.id.et_var);
                    ...
                    set.playTogether(
                            Glider.glide(s, 1200, ObjectAnimator.ofFloat(mTarget, "translationY", 0, dipToPixels(MyActivity.this, -(160 - 3))), new BaseEasingMethod.EasingListener() {
                                ...
                                public void on(float time, float value, float start, float end, float duration) {
                                    ...
                                    mSpeed.drawSpeed(time, duration, value - dipToPixels(MyActivity.this, 60));
                                }
                            })
                    );
                }else{
                    float tension = 0f;
                    if(!et_var.getText().toString().trim().equals("")){
                        tension = Float.parseFloat(et_var.getText().toString().trim());
                    }
                    set.playTogether(
                            Glider.glide(s, 0, dipToPixels(MyActivity.this, -(160 - 3)),1200f,ObjectAnimator.ofFloat(mTarget, "translationY", 0, dipToPixels(MyActivity.this, -(160 - 3))),
                                    new ConvertInterPolator.SpeedListener() {
                                @Override
                                public void on(float time, float value, float start, float end, float duration) {
                                    mHistory.drawPoint(time, duration, value - dipToPixels(MyActivity.this, 60));
                                    mSpeed.drawSpeed(time, duration, value - dipToPixels(MyActivity.this, 60));
    ...
    public static float dipToPixels(Context context, float dipValue) {
        ...
        Log.e("anim",TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, dipValue, metrics)+"px");
